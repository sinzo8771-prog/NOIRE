...
| `node scripts/test-browser.js` | Full Puppeteer regression suite (11 groups / 155 checks: boot, acts, nav, product switcher, concierge CTAs, modal, mobile + viewport sweep, reduced motion, keyboard, analytics, hero variant + tasting interlude, reserve drop, interactivity). Screenshots → `probe-artifacts/browser-regression/`. Requires Chrome + server on :3000 |

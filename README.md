...
| `node scripts/test-browser.js` | Full Puppeteer regression suite (12 groups / 162 checks: boot, acts, nav, product switcher, concierge CTAs, modal, mobile + viewport sweep, reduced motion, keyboard, analytics, hero variant + tasting interlude, reserve drop, interactivity, signature trio, cut-reveal). Screenshots → `probe-artifacts/browser-regression/`. Requires Chrome + server on :3000 |
